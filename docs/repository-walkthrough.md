...
This is a plain-language map of **all 199 tracked files**, including this walkthrough. Each file has its own entry below. The 173-file audited experiment at commit `17b4aaebcf9327ae9967ca45ddfaf16325766993` was followed by this walkthrough, then six interactive-course files, eighteen video files and the preserved final video-quality review. The course adds teaching and verification; it does not change experiment code, data settings, results or the original customer presentation. The current checksum index adds the new course documents and video assets while preserving every previous artifact's fingerprint.
