...
This companion explains every repository file, its inputs and outputs, and when to use it. The current course is **v4**, including the measured confidence-calibration addition. Its [source, checks and downloads](#v4-course) and [96-scene frame map](#v4-scenes) have individual entries. Missing output links are reserved paths, not evidence of successful generation. Before publication, current coverage must reconcile every tracked and nonignored new file and bind actual reviewed media bytes.

The original audited customer package at commit `17b4aaebcf9327ae9967ca45ddfaf16325766993` is historical. Its customer index, quickstart, eight-page briefing, 16-slide deck and ZIP retain their original contents and do not include later calibration. Use the current root README, this guide, v4 video/documents and the [support matrix](support-matrix.md) for today's entry points. The historical quickstart's 54-test count and then-untested Windows/macOS status belong to its snapshot.
...
| Logits / scores | Logits are six raw model outputs. Softmax converts them to six normalized display scores. Optional temperature scaling adds calibrated benchmark scores without changing the winning category; neither score establishes customer-network attack probability. |
...
| File | What it does and when you would open it |
|---|---|
| <a id="file--github-workflows-ci-yml"></a>[ci.yml](../.github/workflows/ci.yml) | Runs six CPU jobs: Ubuntu 24.04, Windows 2022 and macOS 14 with Python 3.10/3.12. Reads the checkout and saved evidence, runs tests, recomputes calibration, checks current v4 and pinned historical v3, and checks page freshness. Full history supplies the immutable v3 blobs. A green job does not repeat GPU training or prove a CPU model backend. |
...
| <a id="file-docs-repository-walkthrough-md"></a>[repository-walkthrough.md](repository-walkthrough.md) | This current individual-file guide with v4 and historical v3 scene maps. The v4 handbook includes its reviewed text. Every file has a unique anchor; listed future outputs do not become completed evidence until the coverage and audit records establish their identities. |
...
| File | What it does and when you would open it |
|---|---|
| <a id="file-docs-customer-README-md"></a>[README.md](customer/README.md) | Frozen original customer-package index and checklist, bound by historical checksums. Its 16-slide deck and eight-page briefing describe that snapshot. Start current work from the root README, this guide, v4 video/documents and support matrix. |
...
The 30-minute lesson is preserved in the [historical course-video-v2 release](https://github.com/buffbeefalo/netmambaplus-reproduction/releases/tag/course-video-v2) and the files listed here. Its source, media, captions, transcript, manifest and receipts describe that version. The [one active watch route](https://buffbeefalo.github.io/netmambaplus-reproduction/video/) is a current navigation surface and selects the separately checked current v4 media; its HTML is not a frozen v2 receipt. The original v2 HTML remains available in Git history. These video releases do not replace the seven original audited customer attachments.

| File | What it does and when you would open it |
|---|---|
| <a id="file-docs-customer-video-course-source-json"></a>[docs/customer/video-course-source.json](customer/video-course-source.json) | Preserved v2 narration and visible teaching for 60 scenes. Contains evidence-bound numbers, the nine original chapter budgets, practice durations and two audio-bound caption corrections. Current v4 authoring uses its own source file. |
...
| <a id="file-docs-customer-demo-video-index-html"></a>[docs/customer/demo/video/index.html](customer/demo/video/index.html) | The single current watch page generated from v4 source and media identities. Provides playback, chapters, a reflowing transcript and v4 downloads. It changes on current publication and is not a frozen v2/v3 receipt or an inference service. |
...
| <a id="file-tools-build_video_page-py"></a>[tools/build_video_page.py](../tools/build_video_page.py) | Reads selected v4 source/media, checks their identities and current reference bytes, and writes the active page with v4 downloads. --source, --media-dir and --output support staging; --check rejects stale HTML. Historical HTML requires a separate output and cannot replace the active route. |
| <a id="file-tools-verify_course_video-py"></a>[tools/verify_course_video.py](../tools/verify_course_video.py) | Defaults to current v4 source/reference, artifact, scene/chapter and caption checks. Optional FFmpeg/Playwright modes measure actual media and browser behavior. --historical-v3 checks unchanged v3 files against pinned Git references; --legacy retains v2. A file-only pass is not human playback acceptance. |
...
## 16. Current v4 course and confidence calibration

V4 keeps one 60-minute lesson with 12 chapters, 96 stable scene IDs and 200 seconds of declared practice. Codex authors the text; Andrew synthetic speech renders it. The current source and measured narration allocation drive the scene map below. Actual encoded duration, layout, speech/caption observations and downloads require their own source-bound records. Current readiness cannot reuse v3 receipts.

The new host-side feature fits one positive temperature per frozen classifier on 1,040 validation flows, then adds calibrated scores and optional accept/defer to native prediction. It changes no weights or winning class. Across the three already-seen 1,041-flow test sets, mean NLL changed from **0.4416 to 0.4172**, Brier from **0.2035 to 0.2012**, and 15-bin ECE from **7.03% to 3.82%**; mean accuracy stayed **86.65%**. Lower is better for these confidence measurements, but they are different metrics, not an operational guarantee.
...
The production path is source → identity-checked speech caches → timed video/frames → matching documents → technical/content checks → coverage/publication. `--source` selects the lesson explicitly for narration, video, documents and page tools. Current defaults use v4; historical verification reads pinned v3 Git blobs without executing old code. The v4 verifier requires the original 13 scientific claims plus ten calibration claims and every tracked/nonignored file.
...
| <a id="file-tools-verify_video_course_v4-py"></a>[tools/verify_video_course_v4.py](../tools/verify_video_course_v4.py) | Current entry point requiring every tracked/nonignored file, useful guide anchors, teaching/paper/CSV coverage, 23 required claims and actual source/media/review identities. --content does not grant readiness; --schema only describes requirements. Human full-watch/all-caption acceptance stays pending. |
...
| <a id="file-docs-customer-video-course-v4-source-json"></a>[docs/customer/video-course-v4-source.json](customer/video-course-v4-source.json) | Current Codex-authored 96-scene source with exact narration, visible examples, citations, practice, voice and one-hour budgets. Includes calibration method/results. Speech, video, captions and documents must bind these bytes; this is neither flow data nor model weights. |
...
### Current coverage and audit records

These entries describe required records. A reserved path does not mean generation or review passed; consult each actual status, method and artifact hash. Human full-watch/listening and all-caption acceptance remain pending.

| File | What the record establishes and how to use it |
|---|---|
| <a id="file-docs-customer-video-course-v4-coverage-json"></a>[docs/customer/video-course-v4-coverage.json](customer/video-course-v4-coverage.json) | Current source-bound inventory and content/visual map: each file’s unique guide anchor, paper/CSV teaching, clusters, worked answers and 23 required quantities. Links actual media and completed technical receipts. A filename or content-only result does not grant readiness. |
| <a id="file-docs-customer-video-verification-v4-md"></a>[docs/customer/video-verification-v4.md](customer/video-verification-v4.md) | Readable current build/review/publication receipt, exact artifacts, commands, observations and limitations. Separates computational checks/Codex review from pending human watch/listening and all-caption acceptance. Includes the current seven-question map. |
| <a id="file-docs-research-video-audit-v4-json"></a>[docs/research/video-audit-v4.json](research/video-audit-v4.json) | Current bundle binding full script/frame review and encoded audio/caption/browser observations to final v4 bytes. Retains uncertainties and scope; cannot borrow v3 acceptance or invent a human-listening pass. |
...
| <a id="file-docs-research-video-v4-claims-json"></a>[docs/research/video-v4/claims.json](research/video-v4/claims.json) | Current visible/spoken quantity review against original evidence and calibration results. Binds source/encoded examples with correct population, units and limits; matching a number alone is insufficient. |
...
### Every current course download and production record

Use the [active watch route](https://buffbeefalo.github.io/netmambaplus-reproduction/video/) or the [versioned v4 release](https://github.com/buffbeefalo/netmambaplus-reproduction/releases/tag/course-video-v4). All format alternatives describe one lesson. During assembly, missing files remain reserved outputs; final records establish their completion.

| File | Purpose, inputs, outputs and use |
|---|---|
| <a id="file-docs-customer-demo-video-v4-NetMambaPlus-one-hour-course-mp4"></a>[docs/customer/demo/video/v4/NetMambaPlus-one-hour-course.mp4](customer/demo/video/v4/NetMambaPlus-one-hour-course.mp4) | Complete current H.264/AAC lesson assembled from reviewed speech, frames, captions, motion and recorded replay footage. Use for offline viewing/presentation; actual decode and review must bind these bytes. |
...
### Current chapter, scene and frame map
...
| <a id="file-tools-verify_video_course_v3-py"></a>[tools/verify_video_course_v3.py](../tools/verify_video_course_v3.py) | Shares strict coverage/claim/media checks with v4. Its historical CLI reads all 341 files at commit 71290440e917105ffa094402d94e24d7e29a3549 as data, rejects local v3 source/coverage/asset drift and keeps the original 13 required claims. V4 uses current inventory plus ten calibration claims. It never executes old code; timing, captions, audio, frames and pending-human boundaries remain enforced. |
