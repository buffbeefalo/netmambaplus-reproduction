...
This is a plain-language map of **all 174 tracked files**, including this walkthrough. Each file has its own entry below. The inventory extends the 173-file audited project at commit `17b4aaebcf9327ae9967ca45ddfaf16325766993` with this guide; the accompanying README and lesson changes add navigation links. The experiment code, data settings, results and customer presentation are unchanged by this documentation addition.

“Tracked” means included in Git and visible in the GitHub repository. The downloaded authors’ code, raw data, locally trained weights, temporary files and Git’s internal history are outside that inventory. Their roles are explained near the end. The existing audited release remains a fixed package of its recorded commit; this later walkthrough is available from the current repository.
...
| [pages.yml](../.github/workflows/pages.yml) | On a push to `main` or manual dispatch, runs tests and package verification, then publishes `docs/customer/demo` to GitHub Pages. That folder contains the replay, learning guide and meeting backups. It does not deploy a live IDS. |
...
## 6. `tests/`: the 54 CPU regression tests

These use temporary examples and controlled substitutes for expensive components. Their purpose is to catch software mistakes and verify rejection behavior. The real GPU experiments have separate evidence.
