...
The [current single-workflow audit](../research/packet-only-handoff.md) records 468 source tests (432 passed, 36 optional skips) and 185 tests from a fresh standalone client export (149 passed, 36 optional skips) on the local Linux environment. The exported client also passed all 17 native GB10 cases without skips. Hosted Windows jobs have one additional skip; each CI run records its own counts.

A fresh native prediction from that packet-only client validated all 20,000 CIC input rows and predicted the first 128. All classes matched, but **76 of 128 strict logit comparisons failed**, with maximum absolute difference 0.006159305572509766. This repeated the result in the [earlier client audit](../research/client-edition-validation.json). It is separate from the original full 25,930-group inference comparison, which retained **two** strict failures. Both used declared relative tolerance `1e-4` and absolute tolerance `1e-6`. Do not combine those counts or claim all numerical comparisons passed.
