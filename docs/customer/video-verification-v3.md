...
Open the watch page and press Play. The chapter controls let you revisit a topic; the complete transcript below the player reflows on smaller screens. Use fullscreen for code and tables, especially on a phone. Download the MP4 for an offline meeting. Open the PowerPoint's Notes view for the matching narration, or use the searchable PDF handbook while explaining the repository. Viewing these files needs no GPU, Python environment or AI account.
...
| Public browser controls | The deployed watch page passes picture and audio playback, all **12** keyboard chapter seeks, near-end playback, all captions, exact MP4 download bytes, and no horizontal overflow at **320, 390, 768 and 1440 pixels**. |
| Portable CI | **169 tests pass in each of six jobs**: Linux, Windows and macOS with Python 3.10 and 3.12. All package, video-readiness and page-freshness checks also pass at release revision `cd028059818f38c51f40589214eac5a6a521ad3e`. [Read the actual workflow](https://github.com/buffbeefalo/netmambaplus-reproduction/actions/runs/34756231815). |
| Published downloads | All **10** new release assets were downloaded anonymously and their complete sizes and SHA-256 hashes matched. All **25** attachments across the four older releases were also downloaded and matched their preserved identities. |
...
The additive [**`course-video-v3`** release](https://github.com/buffbeefalo/netmambaplus-reproduction/releases/tag/course-video-v3) is public at commit **`cd028059818f38c51f40589214eac5a6a521ad3e`**. It is a prerelease and does not replace the original customer release as GitHub's `latest` destination. Earlier releases, their descriptions and all 25 attachments retain their identities. The old customer ZIP retains its original snapshot; the new course downloads and current repository provide this later work.

The [Pages deployment](https://github.com/buffbeefalo/netmambaplus-reproduction/actions/runs/34756376535) succeeded, and anonymous public-player verification completed on **2026-09-13 at 12:12 UTC**. The served page matches the reviewed HTML; its browser encoding plays picture and audio, and its MP4 download matches SHA-256 **`6d12789f1faf6eea70c75247b06a07e4793e275277dc6c817c6245c434186f29`**. An independent HTTP check confirms the retired `/course/` route returns **404**. The structured audit includes the actual CI jobs, download hashes, public browser receipt and deployment identity.

The first v3 CI run found a portability error in one test: the clip helper returned a resolved path, while the test compared Windows short-name or macOS symlink spellings. A local symlink fixture reproduced the failure; resolving the expected path fixed it without changing the renderer or weakening the clip's checksum, geometry or timing checks. The six-job run linked above passed after that correction. The source, narration, MP4, browser copy and companion documents were unchanged by this test correction and the later publication-receipt updates.
