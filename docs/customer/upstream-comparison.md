...
| NetMamba+ | The extension combining bytes, packet sizes and arrival intervals in the original flow experiment. Its native encoder also underlies our separately configured packet adaptation. |
...
They are CSV tables of labeled packet examples. A CSV is a text format with columns and rows, not a running database server. The first scans parsed the files and counted labels and metadata. A later preparation pass validated every stored byte cell, grouped identical payloads globally and prepared both exports for actual packet-model training. The [original profile](evidence/uploaded-csv-profile.json) and [later packet manifest](evidence/packet-study/manifest.json) preserve these separate events.

| File / data source | Observed contents | Role in this project |
|---|---|---|
| `Payload_data_CICIDS2017.csv` | 1,410,255 packet rows; 15 distinct labels; 1,505 columns | Actual CIC-only and joint packet training, after validation and global duplicate grouping |
| `Payload_data_UNSW.csv` | 79,881 packet rows; 10 distinct labels; the same 1,505-column schema | Actual UNSW-only and joint packet training under the same input contract |
| Authors’ CICIoT2022 flow JSON release | 8,323 train + 1,040 validation + 1,041 test examples; six classes | Original flow training, evaluation and later confidence calibration |
...
The exports lack source/destination addresses and ports, a flow identifier, and established packet order within a connection. Reading five adjacent rows cannot recover those missing facts. The trained classifier using these packet tables is therefore an **adaptation**. It does not reproduce the native NetMamba+ flow experiment.

To make compatible flow inputs from captured packets, a separate adapter must establish grouping, direction, order, byte extraction, timing units and padding. That adapter remains unverified here. `predict.py` accepts already assembled native flows. The separate `tools/predict_packets.py` accepts unlabeled packet CSVs with 1,500 payload columns, optionally followed by the four metadata columns. It rejects a label column and loads the packet model's checkpoint.
...
| Packet adaptation | Original classifier supports configurable byte length, sequence length and class head | New wrapper uses 1,500 bytes, no observed size/IAT tokens, 378 positions and a two-class head; original encoder source stays pinned |
| CSV preparation and training | These supplied exports are not established as the paper's training files | Validates both full files; global exact-payload split; retained label contradictions; six 1,000-update native runs and nine diagnostic controls |
| Confidence extension | Raw six-class logits/scores | Validation-fitted temperature scaling and optional accept/defer for frozen flow models; no accuracy change or packet calibration |
...
The original flow architecture and loader stay unchanged. The packet wrapper changes the input contract and output head explicitly, while calling the pinned native encoder. No fictional flows are inferred from CSV rows. The packet protocol discloses its caps and per-subtype support; it cannot claim coverage for subtypes absent from its selected test set. We did not select the original demo seed after seeing final scores. Seed 0 was fixed for the demo in advance, and all three scores remain published. Optional source LDA/class-balancing paths were not separately validated.
...
| [predict.py](../../predict.py) | Classify native flow inputs without known labels; optionally apply matching flow calibration |
| [packet_data.py](../../packet_data.py) / [packet_model.py](../../packet_model.py) | Validate and prepare the two packet CSVs / configure the native encoder's packet input and two-class head |
| [train_packet_model.py](../../tools/train_packet_model.py) / [predict_packets.py](../../tools/predict_packets.py) | Execute the six-arm packet protocol / run saved-checkpoint inference on unlabeled packet rows |
| [review_packet_study.py](../../tools/review_packet_study.py) / [verify_packet_briefing.py](../../tools/verify_packet_briefing.py) | Recompute packet evidence / check the matching presentation's content and reviewed identities |
