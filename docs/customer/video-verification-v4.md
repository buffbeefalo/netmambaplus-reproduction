...
All **226 automated tests** and the 11-command local verification set passed on the GB10 host and again in a fresh, independent local clone of commit `488e83749213d1aeacc07b2295cf823d7269d32f`. That set includes the calibration arithmetic, current and historical video evidence, original package, archived lesson, generated watch page and whitespace checks. The clone contained all 494 files with matching hashes and no production caches, data or model weights. These portable checks run on the CPU; the separate GB10 receipts above demonstrate actual CUDA kernels, a complete classifier weight update/reload and real flow inference.

A separate depth-one clone reproduced a missing-history failure in the unchanged historical-video test. Fetching full history alone made the same test pass. Both CI and Pages now request full Git history, preserving the historical check. This local failure/correction is recorded separately from actual remote CI and publication observations in the same audit. A scoped pass does not claim paper-level accuracy, a production IDS, a completed human full watch, or execution on untested hardware.

The [release-commit CI run](https://github.com/buffbeefalo/netmambaplus-reproduction/actions/runs/34797531297) passed all six combinations of Linux, Windows and macOS with Python 3.10 and 3.12, with 226 tests in each job. The [Pages deployment](https://github.com/buffbeefalo/netmambaplus-reproduction/actions/runs/34797737767) also passed its tests and published the player. An actual browser then played the public v4 video and checked every chapter, captions, seeking and download bytes. All **10 new release downloads** and **35 preserved historical downloads** were fetched anonymously and matched their expected hashes. The current [v4 release](https://github.com/buffbeefalo/netmambaplus-reproduction/releases/tag/course-video-v4) contains the video and matching presentation files.

Publication required recovery from long-upload TLS errors. A temporary workflow verified chunked transfer bytes and the original commit, then uploaded the customer assets. Its last check used a public tag lookup for an unpublished draft and received HTTP 404, so that workflow remains recorded as failed. The draft was independently checked by release ID, published and verified through anonymous downloads. The temporary transfer branches and draft chunks were removed; the final project contains the original checked files. This transport history is retained in the audit and is separate from the successful project CI and Pages runs.
