...
**Historical flow experiment, not a supported client route.** Use [packet setup](packet-study-setup.md) for the current delivery. Commands below belong to the earlier flow study; acquiring its old flow assets now requires explicitly selecting `--inventory configs/assets.json`, because the current default fetcher acquires packet initialization only.
