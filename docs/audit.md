...
| High | The installation command did not force a source build. The bundled installer can download a stock wheel with the same version number, while the native model passes constructor arguments absent from stock Mamba 1.1.1. | The [setup instructions](https://github.com/buffbeefalo/netmambaplus-reproduction/blob/10705a4c11e194e99609fcc6038bbd5341b82448/README.md#research-environment) now force the bundled build, disable the wheel cache for that installation, preserve the pinned dependency stack, and compare installed Python sources with the bundled source. The comparison was tested against a temporary matching copy of all 14 Python files and a deliberately changed module. A real GPU installation was not performed. |
...
- All **10,404 released CICIoT2022 flows** were revalidated: 8,323 training, 1,040 validation and 1,041 test records. Their hashes match the [README evidence record](https://github.com/buffbeefalo/netmambaplus-reproduction/blob/10705a4c11e194e99609fcc6038bbd5341b82448/README.md#source-settings-paper-settings-and-observations). The five train/validation and six train/test shared raw inputs remain disclosed; the records were not changed.
