# Packet-only client handoff implementation plan

**User requirement:** “I want there to be only one route, the one that uses the csvs.” Both repository landing pages, active setup, client verification and the public demo must lead to the joint CICIDS2017/UNSW packet adaptation. The reproduction edition must explain the complete client delivery.

**Architecture:** Preserve the measured packet implementation, protocol and scientific artifacts. Narrow the client's explicit export to packet execution and its dependency closure. Keep historical flow research/media in the source edition with clear historical scope. Add a packet evidence demo, a packet-specific native check and checkpoint-only asset selection. Keep the existing verified source-to-client publication mechanism.

**Implementation authority:** Direct edits and publication are authorized by the user and standing guidance. Council run `f53037cd-44e6-4b36-9930-8d7abaaace81` was convened for this changed scope; its Claude position call returned provider HTTP 529. Preserve its eventual recorded outcome; do not invent consensus or retry the provider call.

## Work and acceptance

- [x] Add meaningful regression cases for checkpoint-only acquisition, packet-only export closure, packet evidence verification and demo arithmetic/escaping/output protection.
- [x] Implement a recorded packet demo using the joint model's full CIC/UNSW test groups; retain contradictory labels and distinguish group metrics from row counts. Test the browser filters and layout.
